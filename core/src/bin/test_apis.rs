...
async fn main() -> Result<()> {
    ...
    match get_env_or_secret("HELIUS_API_KEY") {
        Ok(k) if !k.is_empty() => println!(
            "✅ HELIUS_API_KEY: Configurada correctamente ({} caracteres).",
            k.len()
        ),
    ...
    match load_keypair_secure("WALLET_PRIVATE_KEY") {
        Ok(kp) => println!(
            "✅ WALLET_PRIVATE_KEY: Clave cargada y descifrada exitosamente (Pubkey: {}).",
            kp.pubkey()
        ),
        Err(e) => println!(
            "❌ WALLET_PRIVATE_KEY: Error al desencriptar o cargar - {}",
            e
        ),
    }
