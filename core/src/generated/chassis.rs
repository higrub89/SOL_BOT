...
pub mod chassis_service_server {
    ...
    #[async_trait]
    pub trait ChassisService: std::marker::Send + std::marker::Sync + 'static {
        /// Monitoreo de posiciones
    ...
    #[derive(Debug)]
    pub struct ChassisServiceServer<T> {
        inner: Arc<T>,
                ...
                "/chassis.ChassisService/GetTokenAudit" => {
                    ...
                    impl<T: ChassisService> tonic::server::UnaryService<super::AuditRequest> for GetTokenAuditSvc<T> {
                        ...
                        fn call(
                            ...
                            let inner = Arc::clone(&self.0);
                            let fut = async move {
                                <T as ChassisService>::get_token_audit(&inner, request).await
                            };
                            Box::pin(fut)
                        }
                    }
    ...
    impl<T> Clone for ChassisServiceServer<T> {
        fn clone(&self) -> Self {
            let inner = self.inner.clone();
            Self {
                inner,
    ...
    pub const SERVICE_NAME: &str = "chassis.ChassisService";
    impl<T> tonic::server::NamedService for ChassisServiceServer<T> {
