...
pub async fn cmd_rbuy(
    ...
    if super::CommandHandler::is_hibernating() {
        handler
            .send_message("🛑 Bot en HIBERNACIÓN. Usa `/wake` primero.")
            .await?;
        return Ok(());
    }
    ...
    if parts.len() < 3 {
        ...
        match crate::validation::FinancialValidator::validate_mint(mint, "/rbuy command") {
                    ...
                    .await
                {
                    Ok(res) => {
                        ...
                        if let Err(e) = state_manager.upsert_position(pos).await {
                            handler
                                .send_message(&format!(
                                    "⚠️ <b>DB Error guardando posición:</b> {}",
                                    e
                                ))
                                .await?;
                        }
                        ...
                        if let Err(e) = state_manager.record_trade(trade).await {
                            handler
                                .send_message(&format!(
                                    "⚠️ <b>DB Error registrando trade:</b> {}",
                                    e
                                ))
                                .await?;
                        }
                    ...
                    Err(e) => {
                        handler
                            .send_message(&format!("❌ <b>DEGEN RAYDIUM FAIL:</b> {}", e))
                            .await?;
                    }
                }
            }
            Err(e) => {
                handler
                    .send_message(&format!("❌ <b>MINT VALIDATION ERROR:</b> {}", e))
                    .await?;
            }
        }
    }
...
pub async fn cmd_buy(
    ...
    handler
        .cmd_buy_with_params(mint, amount, slippage, executor, state_manager, feed_tx)
        .await
}
...
pub async fn cmd_buy_with_params(
    ...
    match executor
        .execute_buy_with_custom_params(&valid_mint, kp_opt.as_ref(), amount, 100_000, slippage_bps)
        .await
    {
        Ok(res) => {
            ...
            if res.output_amount > 0.0 {
                ...
                if let Err(e) = state_manager.upsert_position(pos).await {
                    handler
                        .send_message(&format!("⚠️ <b>DB Error:</b> {}\nTx: {}", e, res.signature))
                        .await?;
                } else {
                    // REGISTRAR TRADE
                    let trade = crate::state_manager::TradeRecord {
        ...
        Err(e) => {
            handler
                .send_message(&format!("❌ <b>BUY FAILURE:</b> {}", e))
                .await?;
        }
    }
    Ok(())
