...
pub async fn cmd_panic(
    ...
    if parts.len() < 2 {
        handler
            .send_message("❌ <b>Syntax Error:</b> <code>/panic &lt;MINT&gt;</code>")
            .await?;
        return Ok(());
    }
    ...
    let kp_opt = match load_keypair_secure("WALLET_PRIVATE_KEY") {
        ...
        Err(e) => {
            handler
                .send_message(&format!("⚠️ <b>Key Vault Error:</b> {}", e))
                .await?;
            None
        }
    };
        ...
        .await
    {
        ...
        Err(e) => {
            handler
                .send_message(&format!("❌ <b>CRITICAL FAILURE:</b> {}", e))
                .await?
        }
    }
...
pub async fn cmd_panic_all(
    ...
    if active_positions.is_empty() {
        handler
            .send_message("<b>⚠️ Aborting:</b> No active positions found to liquidate.")
            .await?;
        return Ok(());
    }
    ...
    if let Some(kp) = kp_opt {
        match executor.execute_multi_sell(mints.clone(), &kp, 100).await {
            ...
            Err(e) => {
                handler
                    .send_message(&format!("❌ <b>CRITICAL BUNDLE FAILURE:</b> {}", e))
                    .await?;
            }
        }
    }
