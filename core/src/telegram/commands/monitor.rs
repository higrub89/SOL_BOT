...
pub async fn cmd_track(
    ...
    handler
        .send_message(&format!("🔍 <b>Indexing Asset: {}...</b>", symbol))
        .await?;
    ...
    match scanner.get_token_price(&valid_mint).await {
        ...
        Err(e) => {
            handler
                .send_message(&format!("❌ <b>Tracking Error:</b> {}", e))
                .await?;
        }
    }
...
pub async fn cmd_untrack(
    ...
    if parts.len() < 2 {
        handler
            .send_message("❌ <b>Syntax Error:</b> <code>/untrack &lt;MINT&gt;</code>")
            .await?;
        return Ok(());
    }
    ...
    match state_manager.close_position(mint).await {
        ...
        Err(e) => {
            handler
                .send_message(&format!("❌ <b>DB Fault:</b> {}", e))
                .await?;
        }
    }
    Ok(())
...
pub async fn cmd_update(
    ...
    if parts.len() < 3 {
        handler
            .send_message("❌ <b>Syntax Error:</b> <code>/update &lt;MINT&gt; sl=-X tp=Y</code>")
            .await?;
        return Ok(());
    }
