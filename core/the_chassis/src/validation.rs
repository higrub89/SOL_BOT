...
impl FinancialValidator {
    ...
    pub fn validate_price_change(
        ...
        if change_pct > max_change_percent {
            anyhow::bail!(
                ...
                context, change_pct, max_change_percent, old_price, new_price
            );
        }
    ...
    pub fn validate_liquidity(liquidity_usd: f64, min_liquidity: f64, context: &str) -> Result<f64> {
        ...
        if liquidity_usd < min_liquidity {
            anyhow::bail!(
                ...
                context, liquidity_usd, min_liquidity
            );
        }
    ...
    pub fn validate_price_impact(
        ...
        if price_impact_pct > max_impact {
            anyhow::bail!(
                ...
                context, price_impact_pct, max_impact
            );
        }
    ...
    pub fn parse_amount_safe(amount_str: &str, context: &str) -> Result<u64> {
        let amount = amount_str
            .parse::<u64>()
            .with_context(|| format!("{}: No se pudo parsear cantidad '{}'", context, amount_str))?;
...
mod tests {
    ...
    fn test_validate_price_change() {
        // Cambio normal (10%)
        assert!(FinancialValidator::validate_price_change(
            0.001, 0.0011, 50.0, "test"
        ).is_ok());
        
        // Cambio grande pero dentro del límite (40%)
        assert!(FinancialValidator::validate_price_change(
            0.001, 0.0014, 50.0, "test"
        ).is_ok());
        
        // Cambio demasiado grande (100%)
        assert!(FinancialValidator::validate_price_change(
            0.001, 0.002, 50.0, "test"
        ).is_err());
        
        // Caída demasiado grande (-60%)
        assert!(FinancialValidator::validate_price_change(
            0.001, 0.0004, 50.0, "test"
        ).is_err());
    }
    ...
    fn test_validate_liquidity() {
        assert!(FinancialValidator::validate_liquidity(
            10000.0, 1000.0, "test"
        ).is_ok());
    ...
    fn test_validate_price_impact() {
        assert!(FinancialValidator::validate_price_impact(
            1.5, 5.0, "test"
        ).is_ok());
    ...
    fn test_parse_price_safe() {
        ...
        assert!(FinancialValidator::parse_price_safe("invalid", "test").is_err());
        assert!(FinancialValidator::parse_price_safe("0", "test").is_err());
    }
